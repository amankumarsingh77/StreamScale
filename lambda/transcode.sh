...
send_status_update() {
    ...
    aws sns publish \
        ...
        --message "{\"status\":\"${status}\",\"progress\":${progress},\"duration\":${duration},\"videoKey\":\"${S3_KEY}\",\"taskId\":\"${TASK_ID}\",\"thumbnailUrl\":\"${thumbnail_url}\",\"videoCodec\":\"${video_codec}\",\"audioCodec\":\"${audio_codec}\",\"resolution\":\"${resolution}\"}" \
...
handle_error() {
    ...
    send_status_update "FAILED" "0" "$duration" "" "$video_codec" "$audio_codec" "$resolution"
    cleanup
    exit 1
}
...
if [ $? -ne 0 ]; then
    handle_error "Failed to get video metadata"
fi
...
ffmpeg -i "${VIDEO_FILE}" \
    ...
    -filter:v:0 "scale=-2:480" -c:v:0 libx264 -preset veryfast -crf 23 -c:a:0 aac -b:a 128k \
    -filter:v:1 "scale=-2:720" -c:v:1 libx264 -preset veryfast -crf 23 -c:a:1 aac -b:a 128k \
    ...
    -progress - 2>&1 | while read line; do
        ...
        if [[ $line =~ out_time_ms=([0-9]+) ]]; then
            ...
            if (( current_time - last_update_time >= 30 )) || (( progress - last_progress >= 5 )); then
                send_status_update "TRANSCODING" "$progress" "$duration" "" "$video_codec" "$audio_codec" "$resolution"
...
if [ ${PIPESTATUS[0]} -ne 0 ]; then
    ...
    send_status_update "FAILED" "100" "$duration" "" "$video_codec" "$audio_codec" "$resolution"
fi
...
send_status_update "UPDATE_THUMBNAIL" "0" "$duration" "${R2_CDN_ENDPOINT}/${UPLOAD_DIR}/${OUTPUT_BASE_NAME}/thumbnail.jpg" "$video_codec" "$audio_codec" "$resolution"
...
send_status_update "DONE" "100" "$duration" "${R2_CDN_ENDPOINT}/${UPLOAD_DIR}/${OUTPUT_BASE_NAME}/thumbnail.jpg" "$video_codec" "$audio_codec" "$resolution"